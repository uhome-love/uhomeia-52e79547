...
export default function CampanhasVozContent() {
  const { user } = useAuth();
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="criar">
        <TabsList className="w-full grid grid-cols-3">
          <TabsTrigger value="criar" className="text-xs"><Plus className="h-3 w-3 mr-1" /> Criar</TabsTrigger>
          <TabsTrigger value="ativas" className="text-xs"><Play className="h-3 w-3 mr-1" /> Campanhas</TabsTrigger>
          <TabsTrigger value="historico" className="text-xs"><BarChart3 className="h-3 w-3 mr-1" /> Histórico</TabsTrigger>
        </TabsList>

        <TabsContent value="criar">
          <Card className="p-4 space-y-4">
            <h3 className="text-sm font-bold flex items-center gap-2"><Plus className="h-4 w-4" /> Nova Campanha de Voz</h3>
            <div className="space-y-3">
              ...
              <div>
                ...
                <Input type="number" value={limit} onChange={(e) => setLimit(e.target.value)} className="h-9 text-xs" min="1" max="50" />
              </div>
              <div className="flex items-center gap-2 p-2 bg-amber-50 dark:bg-amber-950 rounded text-xs text-amber-700 dark:text-amber-400">
                ...
                <span>Ligações só entre 9h e 20h BRT (seg-sex). Máx 50 por campanha.</span>
              </div>
              <Button className="w-full gap-2" onClick={handleCreateCampaign} disabled={creating}>
        ...
        <TabsContent value="ativas">
          <ScrollArea className="h-[500px]">
            <div className="space-y-3">
              ...
              {campaigns.filter((c) => ["agendada", "em_andamento"].includes(c.status)).map((c) => {
                ...
                return (
                  <Card key={c.id} className="p-4 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2"><Icon className="h-4 w-4" /><span className="text-sm font-semibold">{c.nome}</span></div>
        ...
        <TabsContent value="historico">
          <ScrollArea className="h-[500px]">
            <div className="space-y-2">
              ...
              {campaigns.filter((c) => c.status === "concluida").map((c) => {
