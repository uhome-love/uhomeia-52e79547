...
import { Loader2, DollarSign, CheckCircle, AlertCircle, Building2 } from "lucide-react";
...
export default function CeoVendasAssinadas() {
  ...
  const loadVendas = async () => {
    ...
    // Get all assinados from pdn_entries for the selected month
    const { data: pdns } = await supabase
      .from("pdn_entries")
      .select("id, nome, und, empreendimento, vgv, status_pagamento, corretor, equipe, gerente_id, mes, data_visita")
    ...
    const result: VendaAssinada[] = pdns.map(p => ({
      ...
      corretor: p.corretor,
      equipe: p.equipe,
      gerente_nome: profileMap.get(p.gerente_id) || "—",
      mes: p.mes,
  ...
  const filtered = empreendimentoFiltro === "all" 
    ? vendas 
    : vendas.filter(v => v.empreendimento === empreendimentoFiltro);
  ...
  const totalSemStatus = totalVgv - totalPago - totalFaltaPagar;
  ...
  // Group by empreendimento for summary
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center gap-2 text-muted-foreground mb-1">
                  ...
                  <span className="text-xs font-medium">Empreendimentos</span>
                </div>
                <p className="text-2xl font-display font-bold text-foreground">{Object.keys(byEmpreendimento).length}</p>
                <p className="text-[10px] text-muted-foreground mt-1">com vendas assinadas</p>
              </CardContent>
            </Card>
          </div>
