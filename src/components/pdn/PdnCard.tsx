...
const TEMP_CONFIG: Record<string, { icon: React.ElementType; label: string; color: string; bg: string; border: string }> = {
  quente: { icon: Flame, label: "Quente", color: "text-red-600", bg: "bg-red-500/10", border: "border-red-500/40" },
  morno: { icon: Thermometer, label: "Morno", color: "text-amber-600", bg: "bg-amber-500/10", border: "border-amber-500/40" },
  frio: { icon: Snowflake, label: "Frio", color: "text-blue-500", bg: "bg-blue-500/10", border: "border-blue-500/40" },
};

const RISCO_CONFIG: Record<RiscoNivel, { emoji: string; color: string; bg: string; border: string; label: string; barColor: string }> = {
  seguro: { emoji: "🟢", color: "text-emerald-600", bg: "bg-emerald-500/10", border: "border-emerald-500/40", label: "Seguro", barColor: "bg-emerald-500" },
  atencao: { emoji: "🟡", color: "text-amber-600", bg: "bg-amber-500/10", border: "border-amber-500/40", label: "Atenção", barColor: "bg-amber-500" },
  risco: { emoji: "🔴", color: "text-red-600", bg: "bg-red-500/10", border: "border-red-500/40", label: "Risco", barColor: "bg-red-500" },
};

const DOCS_CONFIG: Record<string, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  doc_completa: { label: "Docs OK", color: "text-emerald-600", bg: "bg-emerald-500/10", icon: CheckCircle },
  em_andamento: { label: "Em andamento", color: "text-amber-600", bg: "bg-amber-500/10", icon: FileText },
  sem_docs: { label: "Sem docs", color: "text-muted-foreground", bg: "bg-muted", icon: FileText },
};
...
const PdnCard = memo(function PdnCard({ entry, readOnly, onUpdate, onDragStart, onDragEnd, isDragged }: PdnCardProps) {
  ...
  const docs = DOCS_CONFIG[entry.docs_status] || DOCS_CONFIG.sem_docs;
  ...
  const semProximaAcao = isActive && (!entry.proxima_acao || !entry.proxima_acao.trim());
  const vgvFormatted = formatBRL(entry.vgv);
  const objecaoLabel = OBJECAO_OPTIONS.find(o => o.value === entry.objecao_cliente)?.label;

  // Determine card border color based on risk
  const borderColor = isCaiu
    ? "border-l-red-500/60"
    : isAssinado
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div
        ...
        className={`group relative rounded-xl border-l-[3px] border bg-card p-0 cursor-grab active:cursor-grabbing hover:shadow-lg hover:shadow-primary/5 transition-all duration-200 select-none overflow-hidden ${borderColor} ${
      ...
      >
        ...
        <div className="p-3 space-y-2">
          ...
          <div className="flex items-center gap-1.5 text-[10px]">
            <docs.icon className={`h-3 w-3 shrink-0 ${docs.color}`} />
          ...
          {isActive && (
            <div className="space-y-1">
              {entry.proxima_acao && entry.proxima_acao.trim() ? (
                ...
                <Select value="" onValueChange={v => onUpdate(entry.id, { proxima_acao: v })}>
