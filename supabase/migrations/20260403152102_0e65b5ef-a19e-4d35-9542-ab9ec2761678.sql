...
BEGIN
  ...
  WHERE c.data = v_today_date
    ...
    AND (
      (v_target_janela = 'dia_todo' AND c.janela = 'dia_todo')
      OR (v_target_janela <> 'dia_todo' AND (c.janela = v_target_janela OR c.janela = 'dia_todo'))
    )
  ...
  IF v_chosen_auth_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'reason', 'no_corretor_available', 'segmento_id', v_segmento_id, 'janela', v_target_janela);
  END IF;
  ...
  INSERT INTO public.distribuicao_historico (pipeline_lead_id, corretor_id, segmento_id, acao)
  VALUES (p_lead_id, v_chosen_auth_id, v_segmento_id, 'distribuido');

  INSERT INTO public.roleta_distribuicoes (lead_id, corretor_id, segmento_id, janela, status, enviado_em, expira_em, avisos_enviados)
  ...
  IF v_segmento_id IS NOT NULL THEN
    PERFORM public.increment_roleta_fila(v_chosen_profile_id, v_segmento_id, v_today_date);
  END IF;
