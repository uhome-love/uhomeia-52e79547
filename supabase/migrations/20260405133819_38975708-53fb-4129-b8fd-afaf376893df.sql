...
-- Drop the old 3-arg version to avoid ambiguity
DROP FUNCTION IF EXISTS public.distribuir_lead_atomico(uuid, text, uuid);

-- Recreate the single version with p_force
...
BEGIN
  ...
  WHERE c.data = v_today_date
    ...
    -- p_force bypasses na_roleta check
    AND (p_force = true OR cd.na_roleta = true)
    -- On special days, accept ANY janela
    AND (
      v_is_special_day
