...
BEGIN
  ...
  LOOP
    PERFORM criar_notificacao(
      ...
      '⏰ ' || COALESCE(v_lead.nome, 'Lead') || ' — ' || COALESCE(v_lead.stage_nome, 'Pipeline') || ' precisa de atenção',
      v_lead.nome || ' está na etapa "' || v_lead.stage_nome || '" há ' ||
      round(v_lead.minutos_parado / 1440) || ' dias. Atualize o status!',
      jsonb_build_object('lead_id', v_lead.id, 'pipeline_lead_id', v_lead.id, 'lead_nome', v_lead.nome, 'etapa', v_lead.stage_nome),
