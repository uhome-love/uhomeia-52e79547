...
AS $$
DECLARE
  v_stage_id uuid := 'd3843b2f-2fa1-4c31-9129-4eb0ed21f019'; -- Novo Lead (fixed)
...
BEGIN
  ...
  -- Criar pipeline_lead — SEMPRE no stage Novo Lead
  INSERT INTO pipeline_leads (
    ...
    CASE WHEN NEW.atribuido_para IS NOT NULL THEN 'aceito' ELSE 'pendente_distribuicao' END,
    CASE WHEN NEW.atribuido_para IS NOT NULL THEN now() ELSE NULL END
  )
