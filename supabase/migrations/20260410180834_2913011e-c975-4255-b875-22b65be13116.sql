...
-- Fix: Deduplicate brokers in round-robin selection
-- A broker with multiple segment credentials appeared multiple times in the candidate list,
-- causing them to receive multiple leads while others received none.
...
-- Recreate trigger function with same DISTINCT ON dedup fix
