...
BEGIN
  -- Lock the lead row to prevent concurrent distribution
  ...
  -- ==========================================================================
  -- FIX: Aggregate leads_recebidos per CORRETOR (not per row) to handle
  -- multiple janela entries (manhã, tarde, noturna) for the same person.
  -- Also lock the rows with FOR UPDATE to prevent race conditions.
  -- ==========================================================================
  ...
  WHERE rf.data = v_today
    ...
    AND rf.ativo = true
  ORDER BY (
    -- Sum leads_recebidos across ALL rows for this corretor+segmento+date
    SELECT COALESCE(SUM(rf2.leads_recebidos), 0)
  ...
  -- CRITICAL: Resolve auth user_id from profile
  ...
  -- Assign lead using AUTH user_id
  UPDATE pipeline_leads
  ...
  -- ==========================================================================
  -- FIX: Increment leads_recebidos on ALL rows for this corretor+segmento+date
  -- This ensures the counter is consistent regardless of which janela row
