...
-- 1. Replace trigger function to populate imovel_codigo and imovel_url
...
-- 2. Backfill existing leads that have dados_site but missing imovel_codigo/imovel_url
UPDATE pipeline_leads SET
...
WHERE origem = 'site_uhome'
  ...
  AND dados_site->>'imovel_codigo' != ''
  AND dados_site->>'imovel_slug' IS NOT NULL
  AND dados_site->>'imovel_slug' != '';
