...
BEGIN
  ...
  -- KEY FIX: On holidays/Sundays, accept ANY janela (not just dia_todo)
  ...
  WHERE c.data = v_today_date
    ...
    AND (
      (v_is_sunday OR v_is_holiday)
      OR (c.janela = v_target_janela OR c.janela = 'dia_todo')
    )
  ...
  INSERT INTO public.roleta_distribuicoes (
    pipeline_lead_id, corretor_id, segmento_id, janela, data
  ) VALUES (
    p_lead_id, v_chosen_auth_id, v_segmento_id, v_target_janela, v_today_date
  );

  UPDATE public.roleta_fila
  SET leads_recebidos = COALESCE(leads_recebidos, 0) + 1,
      ultimo_lead_em = v_now
