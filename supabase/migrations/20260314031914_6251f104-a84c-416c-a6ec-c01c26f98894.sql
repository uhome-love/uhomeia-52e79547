...
AS $$
  WITH lig AS (
    ...
    WHERE l.data BETWEEN p_start AND p_end AND (p_user_id IS NULL OR l.auth_user_id = p_user_id)
  ...
  neg AS (
    SELECT n.auth_user_id, SUM(n.conta_proposta) AS propostas, SUM(n.conta_venda) AS vendas, SUM(n.conta_perdido) AS perdidos,
  ...
  gest AS (
    ...
    WHERE g.data BETWEEN p_start AND p_end AND (p_user_id IS NULL OR g.auth_user_id = p_user_id)
