...
AS $$
  ...
  historico_agg AS (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE sn.nome IN ('Proposta', 'Negociação')) AS propostas
  ...
  combined AS (
    SELECT 
      ...
      COALESCE(ha.contatos, 0) * 5 
        ...
        + COALESCE(ha.visitas_realizadas, 0) * 50
        + COALESCE(ha.propostas, 0) * 80 AS pontos_total,
      ...
      COALESCE(ha.propostas, 0) AS propostas
    FROM team_members tm
    LEFT JOIN historico_agg ha ON ha.corretor_id = tm.user_id
    WHERE tm.status = 'ativo'
      AND tm.user_id IS NOT NULL
      AND (COALESCE(ha.contatos, 0) + COALESCE(ha.qualificados, 0) + COALESCE(ha.visitas_marcadas, 0) + COALESCE(ha.visitas_realizadas, 0) + COALESCE(ha.propostas, 0)) > 0
  )
